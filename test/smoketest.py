...
def test_run_basic():
    ...
    testfile = Path("/workspace/test_main.py")
    try:
...
def test_session_switch():
    ...
    file_a = Path("/workspace/test_a.py")
    file_b = Path("/workspace/test_b.py")
...
def test_invalid_token():
    ...
    testfile = Path("/workspace/test_bad.py")
    try:
...
def test_parallel_runs():
    ...
    def run_one(i):
        testfile = Path(f"/workspace/test_task_{uuid.uuid4().hex[:8]}_{i}.py")
        files_created.append(testfile)
        try:
            testfile.write_text(f"print('Task {i}')")
            return run_cmd(["octo", "run", str(testfile)])[1]
        except:
            return ""
...
def test_error_in_script():
    ...
    testfile = Path("/workspace/test_fail.py")
    try:
...
def test_large_output():
    ...
    testfile = Path("/workspace/test_spam.py")
    try:
        testfile.write_text("for i in range(1000): print(f'line {i}')")
        code, output = run_cmd(["octo", "run", str(testfile)], timeout=60)
        assert code == 0
...
def test_long_running():
    ...
    testfile = Path("/workspace/test_sleepy.py")
    try:
        testfile.write_text("import time; time.sleep(3); print('done')")
        code, output = run_cmd(["octo", "run", str(testfile)], timeout=10)
        assert code == 0
...
def test_many_files():
    ...
    test_dir = Path("/workspace/test_many_files")
...
def test_run_with_custom_docker():
    ...
    testfile = Path("/workspace/test_docker_test.py")
    try:
        testfile.write_text("import sys; print('python-version:', sys.version)")
        code, output = run_cmd(["octo", "run", str(testfile)], timeout=30)
        assert code == 0
...
def test_run_with_default_docker():
    ...
    testfile = Path("/workspace/test_default_docker.py")
    try:
